    ...
    DecodeAudioData(audioData, needsSoftwareDecode) {
      if (needsSoftwareDecode) {
        return this._iRuntime._WasmDecodeWebMOpus(audioData).then(rawAudio => {
          const audioBuffer = this._audioContext.createBuffer(1, rawAudio.length, 48000);
    ...
    async _Play(e) {
      ...
      if (startTime > 0 && !e.trueClock) {
        if (this._audioContext.getOutputTimestamp) {
          ...
          startTime = startTime - outputTimestamp.performanceTime / 1000 + outputTimestamp.contextTime;
        } else {
          startTime = startTime - performance.now() / 1000 + this._audioContext.currentTime;
        }
      }
...
"use strict";
{
    ...
    SetParam(param, value, ramp, time) {
      switch (param) {
        case 0:
          ...
          break;
        case 6:
          this._params[1] = value / 1000;
          this.SetAudioParam(this._oscGainNode.gain, value / 1000, ramp, time);
          break;
        case 7:
          this._params[2] = value;
  ...
  self.C3AudioDistortionFX = class C3AudioDistortionFX extends AudioFXBase {
    ...
    _Shape(x, threshold, headroom) {
      const maximum = headroom * 1.05 * threshold;
